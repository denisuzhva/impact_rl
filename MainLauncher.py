...
if __name__ == '__main__':
    ...
    for cfg_name in configs:
        ...
        # Check if model, optimizer and experience dumps exist
        print(opt_path)
        if exists(opt_path):
            print("Models, Optimizer and Experience Buffer loaded")
            opt_chkp = torch.load(opt_path)
            for adx in range(n_agents):
                # for model_type in all_model_params.keys():
                model.load_state_dict(torch.load(
                    trained_dump_dir + f'{cfg_name}_{adx}_policy_net.pth'), strict=False)
            with open(trained_dump_dir + f'{cfg_name}_buffer.pkl', 'rb') as buffer_f:
                buffer = pkl.load(buffer_f)
        else:
            print("Models and opt not found! Initiating new training instance")
            opt_chkp = None
            buffer = q_agent.ExperienceReplay(
                trainer_params['dql_params']['replay_start_size'])
